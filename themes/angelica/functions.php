<?php
/**
 * angelica functions and definitions
 *
...
if ( ! function_exists( 'angelica_setup' ) ) :
	/**
	 ...
	 */
	function angelica_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on angelica, use a find and replace
		 * to change 'angelica' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'angelica', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'angelica' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'angelica_custom_background_args',
				array(
...
add_action( 'after_setup_theme', 'angelica_setup' );
 ...
 */
function angelica_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'angelica_content_width', 640 );
}
add_action( 'after_setup_theme', 'angelica_content_width', 0 );
 ...
 */
function angelica_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'angelica' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'angelica' ),
...
add_action( 'widgets_init', 'angelica_widgets_init' );
...
function angelica_scripts() {
	...
	wp_register_script( 'angelica-carrousel-2', get_template_directory_uri() . '/js/carrousel-2.js', array(), filemtime(get_template_directory() . "/js/carrousel-2.js"), true );
	...
	if( is_front_page()){
		...
		wp_enqueue_script( 'angelica-carrousel-2');
    }
...
add_action( 'wp_enqueue_scripts', 'angelica_scripts' );
